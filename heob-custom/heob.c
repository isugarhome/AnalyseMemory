...
CODE_SEG(".text$7") void mainCRTStartup( void )
{
  ...
  while( args )
  {
    ...
    switch( args[1] )
    {
      ...
      case 'o':
        if( ad->outName ) break;
        ad->outName = getQuotedStringOption( args+2,heap,&args );
        break;

      case 'x':
        if( ad->xmlName ) break;
        ad->xmlName = getQuotedStringOption( args+2,heap,&args );
        break;

      case 'v':
        if( ad->svgName ) break;
        ad->svgName = getQuotedStringOption( args+2,heap,&args );
        break;
