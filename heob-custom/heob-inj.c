  ...
  if( rd->opt.dlls )
  {
    ...
    if( idd->Size )
    {
      ...
      for( i=0; i<number; i++ )
      {
        ...
        // 如果前缀长度和pointPos先沟通呢，且不是以forwardName开头的则直接跳过
...
#define REP_FUNC_(func) \
  ...
  for( ; rd->mod_d<rd->mod_q; rd->mod_d++ )
  {
    ...
    if( !rd->mod_d && rd->opt.handleException<2 )
    {
      ...
      {
       // 如果没有dll_msvcrt则清空splits和freeds
        rd->noCRT = noCRT = 2;
      ...
      // 如果有injected且开启了保护检测，则获取路径和临时路径相关的函数
    ...
    case HEOB_LEAK_RECORDING_SHOW:
      {
        ...
        for( i=0; i<=SPLIT_MASK; i++ )
        {
        // lgz comment: comment it and let it show all leak's memory
        #if 0
          int j;
          splitAllocation *sa = rd->splits + i;
          int alloc_q = sa->alloc_q;
